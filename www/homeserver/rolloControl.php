...
else if ($action=="editSort")
{
  ...
  foreach ((array)$parts as $actId)
  {
    ...
    else
    {
      ...
      MYSQL_QUERY("update guiControlsSaved set sort='$sort',name='$name',value='$position' where id='$actId' limit 1")  or die(MYSQL_ERROR());
    }
  }
}
...
function getLastStatus($lastStatusId)
{
  ...
  $erg3 = MYSQL_QUERY("select functionData,fktId from udpCommandLog where senderObj='$objectId' and (fktId='$statusId'||fktId='$evStopId'||fktId='$evOpenId') and id>'$lastStatusId' order by id desc limit 1") or die(MYSQL_ERROR());
